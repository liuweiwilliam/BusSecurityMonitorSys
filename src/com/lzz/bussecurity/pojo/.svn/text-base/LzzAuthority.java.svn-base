...
public class LzzAuthority {
	...
	public LzzAuthority(boolean need_id){
		if(need_id){
			this.id = LzzIDMgr.self().getID();
		}
	}
	
	public LzzAuthority(LzzAuthority obj){
	    id = obj.getId();
	...
	public LzzAuthority clone(){
		LzzAuthority rslt = new LzzAuthority(false);
		rslt.id = this.getId();
	...
	public void constructWith(LzzAuthority pro) {
