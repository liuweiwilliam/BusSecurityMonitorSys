...
public class LzzRepair {
	...
	public LzzRepair(boolean need_id){
		if(need_id){
			this.id = LzzIDMgr.self().getID();
		}
	}
	
	public LzzRepair(LzzRepair obj){
	    id = obj.getId();
	...
	public LzzRepair clone(){
		LzzRepair rslt = new LzzRepair(false);
		rslt.id = this.getId();
	...
	public void constructWith(LzzRepair pro) {
